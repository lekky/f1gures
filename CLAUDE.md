...
- **`fetch-fastf1.yml`** - deploys **FastF1 session data** for the Weekend Analysis pages. Runs **every 15 min Fri-Sun (+ Mon 0-7 UTC)** with a two-stage gate: `scripts/fastf1-pending.mjs` (Node built-ins, <1s - keep its timing constants in sync with the Python script) exits the run after checkout when no recently-finished session is missing, so ~90% of polls bill ~20-30s; only a pending session pays for pip + `scripts/fetch-fastf1.py --auto` + commit, and only an actual data change pays for npm ci + build + FTP. `.fastf1-cache/` is **not** persisted across CI runs on purpose: FastF1 caches the empty "not ready" response from polling a session before its archive publishes, and a restored cache re-serves that stale empty forever (it stranded 2026 Belgian FP2). Net: race pages grow their session tabs within ~15 min of FastF1 publishing a session (itself ~30-60 min after it ends).
