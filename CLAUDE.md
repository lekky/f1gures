...
- `robots.txt`, `sitemap.xml` — SEO. Sitemap is generated by `scripts/generate-sitemap.mjs` (parses `js/data.js` for drivers, teams, circuits, calendar rounds). Re-run it whenever you add/remove a driver, team, circuit or round.
- `favicon.svg`, `site.webmanifest` — favicon + PWA manifest. Both linked from every `*.html`.
...
- **SEO meta**: each `*.html` has its own `<title>`, `<meta name="description">`, `<link rel="canonical">`, OG/Twitter tags, and a JSON-LD breadcrumb script in `<head>`. Detail screens (`driver`, `race`, `circuit`, `team`) call `useSeo({ title, description, canonicalPath, ogType, jsonLd })` from `shell.jsx` to swap title/description/canonical and inject a `Person`/`SportsEvent`/`Place`/`SportsTeam` JSON-LD block once data loads. Canonical origin is hard-coded as `https://f1gures.app`. When you add a new entity (driver/team/circuit/round), update `js/data.js` and re-run `node scripts/generate-sitemap.mjs`.
- **Boot skeleton**: every `*.html` body starts with `<div id="root"><header class="boot-skeleton"><h1>…</h1>…</header><noscript>…</noscript></div>`. React's `createRoot().render()` replaces `#root`'s contents on hydrate, so the skeleton only flashes briefly. It exists so crawlers and JS-disabled users see a real H1 + intro on first paint. Keep it semantically meaningful when adding new pages.
