...
- **`fetch-fastf1.yml`** - deploys **FastF1 session data** for the Weekend Analysis pages. Crons bracket race-weekend session windows (Fri 13-21, Sat/Sun 8-23 UTC, Mon 05:00 catch-all): pip-installs FastF1 (cached), runs `scripts/fetch-fastf1.py --auto` (fetches any finished session missing from `public/data/fastf1/`), commits, then npm ci + build + FTP **gated on data actually changing** - a no-op poll costs ~1-2 min. `.fastf1-cache/` rides an actions cache so within-weekend reruns don't redownload telemetry. Race pages grow their session tabs ~an hour after each session ends.
