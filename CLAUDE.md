...
### Social posts
- `scripts/social/` - the daily social-post pipeline. `sources.mjs` (memoized archive readers), `angles.mjs` (13 angles + a deterministic two-stage weighted draw seeded by the date, with key/subject/angle cooldowns), `caption.mjs` (per-angle copy, hashtags, alt text), `cardkit.mjs` + `card.mjs` (Satori card renderer: four layouts, brand type + palette), `history.mjs` (`data/social/history.json`, the committed anti-repetition log), `publish/metricool.mjs` (scheduler client). Entry points are `scripts/build-social-post.mjs` and `scripts/publish-social-post.mjs`; the schedule is `.github/workflows/social-post.yml`. Tested (`scripts/social/social.test.js` - pure logic only, since the archive is gitignored and absent in CI).
...
- **`deploy.yml` excludes `social/` from its `lftp mirror --delete`** - the daily cards are uploaded there by `social-post.yml` and are not part of `dist/`, so without the exclusion every deploy would delete the image the live post points at.
