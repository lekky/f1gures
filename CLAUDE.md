...
Note: the deploy workflows don't run tests themselves; the vitest suite runs in a separate **`ci.yml`** workflow on every PR and on pushes to `main` (both with a `paths-ignore` for docs/reference/tooling-only changes). That check is **advisory only** - this repo is private on a personal/Free plan, where GitHub does not enforce branch protection or rulesets, so a red suite can still be merged (look at the check before merging). Real enforcement would need the repo to go public or move to GitHub Pro/Team (tracked in docs/tech-debt.md). The feedback worker is deployed separately by hand (see Feedback worker above).
