...
### Social posts
- `scripts/social/` - the social-post pipeline. **`config.mjs` holds every knob** (post time, live post time, timezone, networks, draft mode, batch horizon, link tagging) - change it there, nowhere else. `sources.mjs` (memoized archive readers), `angles.mjs` (13 angles + a deterministic two-stage weighted draw seeded by the date, with key/subject/angle cooldowns), `caption.mjs` (per-angle copy, hashtags, alt text), `cardkit.mjs` + `card.mjs` (Satori card renderer: six skeletons, brand type + palette), `history.mjs` (`data/social/history.json`, the committed anti-repetition log), `pending.mjs` (`data/social/pending.json`, the MCP hand-off queue), `publish/metricool.mjs` (REST client, `api` route only). Entry points are `scripts/build-social-post.mjs` (`--days=N` batch-builds a fortnight; no flag builds one date) and `scripts/publish-social-post.mjs`. `.github/workflows/social-post.yml` runs **two jobs**: a fortnightly `batch` that fills the Metricool calendar with everything knowable in advance, and a twice-daily `live` job (17:00 and 23:00 UTC, so both European and American race finishes are caught the same day) restricted to result angles (pole/sprint/podium), which cannot be scheduled ahead and publish `asap` rather than at the evening slot. The batch skips the days around a race so the two never collide. Tested (`scripts/social/social.test.js` - pure logic only, since the archive is gitignored and absent in CI).
