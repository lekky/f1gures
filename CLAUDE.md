...
Routine deploys run on the user's **Coolify VPS**, not on GitHub Actions. One long-lived container (`ops/Dockerfile`, **read `docs/ops-vps.md`**) polls `main` every 10 minutes and runs `ops/loop.sh`: hard-sync the clone → fetch the current season from Jolpica and commit + push the bundle if it changed → if HEAD differs from the last deployed sha, `npm ci` (only when the lockfile changed) + `npm run build` + a `dist/` file-count sanity check + **SFTP** mirror (`lftp`, 10 parallel transfers, over the account's SSH access — the Hostmedia cPanel jail has no `rsync`) → once a day, `ops/watchdog.sh` (the team-principals Wikipedia check). Gitignored build caches (`node_modules`, `public/images/og`, `public/data/archive`) persist in the container's `/data` volume, so OG images only regenerate when their inputs change. A failed build/upload is not retried until `main` moves again (`--force` overrides). This replaced the Actions crons, which were exhausting the free minutes (the weekend 10-minute Jolpica poll alone cost ~1,200 min/month, mostly no-ops). Latency: a merge or data push is live within one tick + build (~15–25 min total). The scripts run *from the clone*, so merging a change to `ops/*.sh` takes effect on the next tick; only `ops/Dockerfile` changes need a redeploy in Coolify. **Read `docs/deploy-ssh.md`** for the deploy key setup.

The three workflows that used to do this are now **`workflow_dispatch`-only fallbacks** for when the VPS is down (they keep the same SFTP recipe and repo secrets):

- **`deploy.yml`** - build current `main` + SFTP mirror. Always deploys when dispatched.
- **`refresh-current-season.yml`** - Jolpica fetch + commit + build + deploy.
- **`check-principals.yml`** - the watchdog, opening one deduped issue on drift.

- **`fetch-fastf1.yml`** - **dispatch-only fallback; does NOT auto-run.** The FastF1 fetch cannot run from any datacenter IP: F1's live-timing API refuses GitHub's runners **and the VPS** (verified 2026-09-01 on a completed session: "Failed to load session info data!" from both, while a residential IP loads it fine). So the automated FastF1 fetch runs **locally** via `scripts/fetch-and-deploy-local.ps1` on Windows Task Scheduler (every ~15 min Fri-Sun): it hard-syncs a dedicated clone (`C:\Users\rotsm\f1gures-fastf1-bot`, never the dev checkout) to `origin/main`, runs `scripts/fetch-fastf1.py --auto`, and if new session JSON appeared, commits and pushes to `main`. That's all it does now - the VPS loop notices `main` moved and builds; it no longer dispatches `deploy.yml`. **Backfilling old rounds is also a local job** — run `python scripts/fetch-fastf1.py <year> <round>` on a residential machine and commit.

Note: none of the deploy paths run tests; the vitest suite runs in a separate **`ci.yml`** workflow on every PR and on pushes to `main` (with a `paths-ignore` for docs/reference/tooling-only changes, incl. `ops/`). That check is **advisory only** - this repo is private on a personal/Free plan, where GitHub does not enforce branch protection or rulesets, so a red suite can still be merged (look at the check before merging). Real enforcement would need the repo to go public or move to GitHub Pro/Team (tracked in docs/tech-debt.md). The feedback worker is deployed separately by hand (see Feedback worker above).

The first build after a heavy PR (lots of new routes, or a CSS edit that re-hashes every HTML's `?v=` cache-bust) takes longer on the 2-vCPU VPS and the SFTP mirror re-uploads the whole tree; if the upload dies mid-way (`ECONNRESET`), `docker exec <ctr> bash /data/repo/ops/loop.sh --force` re-runs it idempotently.
