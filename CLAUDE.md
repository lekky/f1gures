...
- `src/components/islands/WorldMap.jsx` - the `/map/` world-map island. Renders a choropleth of F1 nations from `src/data/worldGeo.js` (Natural Earth 1:110m outlines keyed by ISO-2, generated once by `scripts/dev/geo-to-svgpaths.mjs` from `data/geo/world-110m.geojson` - NOT in prebuild) plus `_countries.json`; metric chips switch the shaded stat, micro-states (Monaco, Liechtenstein) render as markers, and a country panel links out to the filtered `/drivers/?nat=` listing. No runtime map/geo library ships.
...
- `src/pages/map.astro` - `/map/` interactive world map; static hero + the `WorldMap` island. Shades every F1 nation by its drivers' achievements (wins/poles/podiums/titles/driver-count/DNFs, switchable), with a country detail panel. Fed by `_countries.json`; the SVG geometry is a self-contained, no-library asset in `src/data/worldGeo.js` (see below). Linked from the Stats hub.
...
- `public/data/archive/` (**gitignored, generated by prebuild**):
  ...
  - `_countries.json` - all-time driver achievements aggregated by the driver's home nation (ISO-2): per country `{ drivers, wins, poles, podiums, championships, dnfs }` plus the top driver per metric. Computed by the final pass of `build-archive.mjs` (see `scripts/countryStats.mjs`, tested) from the fully-merged driver docs. Consumed by the `/map/` world-map island.
...
## Useful URL flags
- `?nat=<demonym>` on `/drivers/` - pre-filters the all-time roster to one nationality (e.g. `/drivers/?nat=British`), shown as a dismissible pill. Used by the `/map/` country panel's "See all … drivers" link. Exact demonym match via `filterItems`.
