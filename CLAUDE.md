...
- **Weekend Analysis** - `src/components/islands/RaceWeekendIsland.jsx` + `src/components/islands/raceweekend/` (`derive.js` pure lap math [tested], `charts-race.jsx`/`charts-quali.jsx` hand-rolled SVG charts, `primitives.jsx`, `results.jsx`, `vizdefs.jsx` chart registry, `share.js` branded-PNG exporter in 16:9 / 1:1 / 9:16). Mounted via `src/components/RaceWeekendBody.astro` on race pages where `public/data/fastf1/<year>/<round>/index.json` exists at build time. Session tabs (countdowns for upcoming sessions) + per-session results + the dark Visualisation Explorer (~30 charts), deep-linkable via `?session=&viz=`. Results blocks use theme tokens; the explorer card is **always dark** (`--vx-*` vars, deliberately not remapped by `html.light`). See `docs/fastf1-pipeline.md`.
