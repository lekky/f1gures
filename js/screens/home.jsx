...
function HomeScreen() {
  ...
  // A season is "historic" once there's no upcoming race left in it. We render
  // a different hero in that case — countdown and session schedule are
  // meaningless when the championship is already decided.
  ...
  return (
    <div className={`page ${mob ? 'page-mob' : ''}`}>
      {isHistoric ? <SeasonAtGlance D={D} cal={cal} standings={standings} mob={mob} /> : <NextRacePanel D={D} cal={cal} mob={mob} />}
...
// ─── Hero panel: Next Race (current season) ───────────────────────────────
function NextRacePanel({ D, cal, mob }) {
...
// ─── Hero panel: Season at a Glance (historic season) ─────────────────────
function SeasonAtGlance({ D, cal, standings, mob }) {
