...
- [features.md](features.md) - every route, what it renders, and which parts are React islands vs static Astro; its **View transitions** section is the rule book for inline scripts (`data-astro-rerun` vs once-per-session) - **read before adding any `<script>`**
...
- [ops-vps.md](ops-vps.md) - the Coolify VPS ops loop that builds + deploys the site and refreshes season data (replaced the Actions crons) - **read before touching `ops/` or the deploy workflows**
- [deploy-ssh.md](deploy-ssh.md) - the Hostmedia SSH deploy key setup used by both the VPS loop and the dispatch-only `deploy.yml` fallback
