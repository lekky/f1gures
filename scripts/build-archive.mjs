...
// ─── ARCHIVE_MAX_YEAR guard ───────────────────────────────────────────
// The newest year with prerendered /races/<y>/<r>/ pages, derived from the now
// fully-populated races index (Ergast + hand-curated bundles + upcoming holding
// pages). The three url builders (shared.jsx, DriverPage, CircuitPage) import
// this so a year rollover is automatic: URLs link straight to a real page for
// any year ≤ this and fall back to the /race.html redirect beyond it. Only
// rewritten when the value changes, so no spurious diff on ordinary builds.
{
  const archiveMaxYear = racesIndex.reduce((m, r) => Math.max(m, r.year), 0);
  const metaPath = join(ROOT, 'src', 'data', 'archiveMeta.js');
  const contents =
    ...
    `export const ARCHIVE_MAX_YEAR = ${archiveMaxYear};\n`;
  const prev = existsSync(metaPath) ? readFileSync(metaPath, 'utf8') : null;
  if (prev !== contents) {
    writeFileSync(metaPath, contents);
    console.log(`[archive] wrote src/data/archiveMeta.js (ARCHIVE_MAX_YEAR=${archiveMaxYear})`);
  }
}
